...
template <typename T>
std::array<T, 2> operator*(std::array<T, 2> arr, T i) {
  std::array<T, 2> res;
  res.at(0) = arr.at(0) * i;
  res.at(1) = arr.at(1) * i;
  return res;
...
template <typename T>
T nCr(const T &n, const T &r) {
  return fact(n) / (fact(r) * fact(n - r));
}

template <typename T>
std::vector<T> linspace(const T &start_in, const T &end_in, T num_in) {
  // Define the vector to return
  std::vector<T> linspaced;
  T start = static_cast<T>(start_in);
  T end = static_cast<T>(end_in);
  // If you want to divide the vector in 1 only element, return the starting
  // point
  if (num_in == 1) {
