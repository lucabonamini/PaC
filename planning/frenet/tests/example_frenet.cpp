...
int main() {
  ...
  while (iteration < MAX_ITERATION) {
    ...
    auto output = Frenet::planFrenetPath(input);
    auto dist_from_goal =
        std::sqrt((output->best_path.x.at(0) - rx.back()) *
                      (output->best_path.x.at(0) - rx.back()) +
                  (output->best_path.y.at(0) - ry.back()) *
                      (output->best_path.y.at(0) - ry.back()));
    ...
    input.robot_state = RobotState{.x = output->best_path.x.at(1),
                                   .y = output->best_path.y.at(1),
                                   .yaw = output->best_path.yaw.at(1),
                                   .s_long = output->best_path.s.at(1),
                                   .v_long = output->best_path.s_d.at(1),
                                   .a_long = output->best_path.s_dd.at(1),
                                   .s_lat = output->best_path.d.at(1),
                                   .v_lat = output->best_path.d_d.at(1),
                                   .a_lat = output->best_path.d_dd.at(1)};
    iteration++;
    ...
    for (const auto &path : output->paths_list) {
      for (size_t i = 0; i < path.x.size(); ++i) {
        cv::circle(bg,
                   cv_offset(path.x.at(i), path.y.at(i), bg.rows),
                   40,
                   cv::Scalar(0, 255, 0),
                   -1);
      }
    }
    for (size_t i = 0; i < output->best_path.x.size(); ++i) {
      cv::circle(bg,
                 cv_offset(output->best_path.x.at(i),
                           output->best_path.y.at(i),
                           bg.rows),
                 40,
                 cv::Scalar(255, 0, 0),
                 -1);
    ...
    cv::circle(bg,
               cv_offset(output->best_path.x.front(),
                         output->best_path.y.front(),
                         bg.rows),
               50,
               cv::Scalar(0, 255, 0),
               -1);
    ...
    cv::putText(
        bg,
        "Speed: " +
            std::to_string(output->best_path.s_d.at(1) * 3.6).substr(0, 4) +
            "km/h",
    ...
    cv::imwrite("/home/nvidia/WIP_Projects/pngs/" + int_count + ".png", outImg);
  }
  return 0;
}